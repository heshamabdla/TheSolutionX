...
internal class LoginLocalDS(private val storageKV: IStorageKeyValue) : ILoginLocalDS {
    ...
    override suspend fun saveLogin(login: LoginEntity) {
        ...
        val encryptedText= Base64.getEncoder().encodeToString(cryptoData.encrypt(byteToken))
        storageKV.saveEntry(StorageKeyEnum.ACCESS_TOKEN, encryptedText)

        val byteUserByte= login.user.toJson().toByteArray()
        val encryptedUser= Base64.getEncoder().encodeToString(cryptoData.encrypt(byteUserByte))
        storageKV.saveEntry(StorageKeyEnum.USER, Gson().toJson( encryptedUser))
    }

    override suspend fun getUser(): UserEntity {

       val json= cryptoData.decrypt(Base64.getDecoder().
    ...
    override suspend fun getAccessToken(): String {
       val token= cryptoData.decrypt(Base64.getDecoder().
